...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    ...
    public class LightLoop
    {
        ...
        public void NewFrame(FrameSettings frameSettings)
        {
            ...
            m_EnableContactShadow = m_FrameSettings.contactShadows && m_ContactShadows.enable && m_ContactShadows.length > 0;
            m_indirectLightingController = VolumeManager.instance.stack.GetComponent<IndirectLightingController>();

            // Cluster
            {
                var clustPrepassSourceIdx = m_FrameSettings.bigTilePrepass ? ClusterPrepassSource.BigTile : ClusterPrepassSource.None;
                var clustDepthSourceIdx = ClusterDepthSource.NoDepth;
                if (k_UseDepthBuffer)
                {
                    if (m_FrameSettings.msaa)
            ...
            {
                s_GenListPerTileKernel = buildPerTileLightListShader.FindKernel(m_FrameSettings.bigTilePrepass ? "TileLightListGen_SrcBigTile_FeatureFlags" : "TileLightListGen_FeatureFlags");
                s_GenListPerTileKernel_Oblique = buildPerTileLightListShader.FindKernel(m_FrameSettings.bigTilePrepass ? "TileLightListGen_SrcBigTile_FeatureFlags_Oblique" : "TileLightListGen_FeatureFlags_Oblique");

            }
            else
            {
                s_GenListPerTileKernel = buildPerTileLightListShader.FindKernel(m_FrameSettings.bigTilePrepass ? "TileLightListGen_SrcBigTile" : "TileLightListGen");
                s_GenListPerTileKernel_Oblique = buildPerTileLightListShader.FindKernel(m_FrameSettings.bigTilePrepass ? "TileLightListGen_SrcBigTile_Oblique" : "TileLightListGen_Oblique");
            }
        ...
        public bool NeedResize()
        {
            return s_LightList == null || s_TileList == null || s_TileFeatureFlags == null ||
                (s_BigTileLightList == null && m_FrameSettings.bigTilePrepass) ||
        ...
        public void AllocResolutionDependentBuffers(int width, int height, bool stereoEnabled)
        {
            ...
            if (m_FrameSettings.bigTilePrepass)
            {
        ...
        public bool GetEnvLightData(CommandBuffer cmd, HDCamera hdCamera, HDProbe probe, DebugDisplaySettings debugDisplaySettings)
        {
            ...
            {
                if (!hdCamera.frameSettings.realtimePlanarReflection)
                    break;
            ...
            ReflectionProbeCullResults reflectionProbeCullResults, DensityVolumeList densityVolumes, DebugDisplaySettings debugDisplaySettings)
        {
            ...
            m_enableBakeShadowMask = m_enableBakeShadowMask && hdCamera.frameSettings.shadowMask;
        ...
        public void BuildGPULightListsCommon(HDCamera hdCamera, CommandBuffer cmd, RenderTargetIdentifier cameraDepthBufferRT, RenderTargetIdentifier stencilTextureRT, bool skyEnabled)
        {
            ...
            if (m_FrameSettings.fptl)
            {
                ...
                if (enableFeatureVariants)
                {
                    ...
                    if (!m_FrameSettings.computeMaterialVariants)
                    {
            ...
            if (enableFeatureVariants)
            {
                ...
                if (m_FrameSettings.computeMaterialVariants)
                {
                    ...
                    uint baseFeatureFlags = 0;
                    if (!m_FrameSettings.computeLightVariants)
                    {
        ...
        public void PushGlobalParams(HDCamera hdCamera, CommandBuffer cmd)
        {
            using (new ProfilingSample(cmd, "Push Global Parameters", CustomSamplerId.TPPushGlobalParameters.GetSampler()))
            {
                ...
                cmd.SetGlobalInt(HDShaderIDs._EnableSSRefraction, hdCamera.frameSettings.roughRefraction ? 1 : 0);
        ...
        public void RenderScreenSpaceShadows(HDCamera hdCamera, RTHandleSystem.RTHandle deferredShadowRT, RenderTargetIdentifier depthTexture, int firstMipOffsetY, CommandBuffer cmd)
        {
            ...
            using (new ProfilingSample(cmd, "Screen Space Shadow", CustomSamplerId.TPScreenSpaceShadows.GetSampler()))
            {
                ...
                kernel = hdCamera.frameSettings.msaa ? s_deferredContactShadowKernelMSAA : s_deferredContactShadowKernel;
                ...
                cmd.SetComputeTextureParam(screenSpaceShadowComputeShader, kernel, hdCamera.frameSettings.msaa ? HDShaderIDs._CameraDepthValuesTexture : HDShaderIDs._CameraDepthTexture, depthTexture);
            ...
            LightingPassOptions options)
        {
            ...
            if (m_FrameSettings.tileAndCluster && m_FrameSettings.computeLightEvaluation && options.outputSplitLighting)
            {
            ...
            string sLabel = m_FrameSettings.tileAndCluster ?
            ...
            {
                // Compute path
                if (m_FrameSettings.tileAndCluster && m_FrameSettings.computeLightEvaluation)
                {
                ...
                else // Pixel shader evaluation
                {
                    int index = GetDeferredLightingMaterialIndex(options.outputSplitLighting ? 1 : 0,
                            m_FrameSettings.tileAndCluster ? 1 : 0,
                    ...
                    else
                    {
                        ...
                        if (!m_FrameSettings.subsurfaceScattering)
                        {
        ...
        public void RenderForward(Camera camera, CommandBuffer cmd, bool renderOpaque)
        {
            ...
            if (!m_FrameSettings.tileAndCluster)
            {
            ...
            else
            {
                ...
                bool useFptl = renderOpaque && m_FrameSettings.fptlForForwardOpaque;
